...
import { Defaults, Values } from '../constants';
...
const monthList = (index: number) => monthNames[index] || '';
...
export const projectionForKonventional = (
  ...
  return parseInt(String(+imbalBalik > 0 ? imbalBalik : 0), 10);
};
...
export const projectionForSyariah = (
  ...
  return parseInt(String(+totalPengembalian > 0 ? totalPengembalian : 0), 10);
};
...
export const checkFundingNominalOd = value => {
  ...
  const regex = /^[0-9]*$/gm;
  const result = regex.test(String(value / 1000000));
  return result;
};
