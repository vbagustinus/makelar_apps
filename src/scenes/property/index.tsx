...
function PropertyScreen() {
  ...
  const styles: any = React.useMemo(() => createStyles(colors), [colors]);
  ...
  const createDummyPropertyData = usePropertyStore(
    state => state.createDummyPropertyData,
  );
  ...
  const resolvePropertyDocId = React.useCallback(
    (property: any) =>
      property?.firestoreId || property?.id || property?.propertyId || null,
    [],
  );
  ...
  React.useEffect(() => {
    if (pendingDeleteId && !globalLoading) {
      setPendingDeleteId(null);
    }
  }, [pendingDeleteId, globalLoading]);
  ...
  const emptyStateMessage = React.useMemo(() => {
    ...
    if (filterStatus !== 'all') {
      return 'Belum ada properti pada filter ini.';
    }
    return 'Belum ada properti.';
  }, [searchQuery, filterStatus]);

  const emptyStateDescription = React.useMemo(() => {
    if (searchQuery.trim() || filterStatus !== 'all') {
      return 'Coba ubah kata kunci atau filter yang dipilih.';
    }
    return 'Tambahkan properti baru dengan tombol "Tambah".';
  }, [searchQuery, filterStatus]);
  const handleAdd = () => {
  ...
  const handleCreateDummy = () => {
    Alert.alert(
      'Buat Dummy Property',
      'Tambahkan 1 data dummy ke Firestore untuk uji insert?',
      [
        { text: 'Batal', style: 'cancel' },
        {
          text: 'Buat',
          onPress: () => {
            createDummyPropertyData();
          },
        },
      ],
    );
  ...
  const handleEdit = property => {
    navigation.push('EditPropertyScreen', {
      ...property,
      id: resolvePropertyDocId(property),
    });
  };

  const handleShare = async property => {
    try {
      const propertyId = resolvePropertyDocId(property);
  ...
  const handleDelete = property => {
    const docId = resolvePropertyDocId(property);
    if (!docId) {
      Alert.alert('Gagal', 'ID properti tidak valid.');
      return;
    }
      ...
      [
        { text: 'Batal', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            deletePropertyData(docId, imageUrls, property);
          },
        },
      ],
    );
            ...
            {isOwner && (
              <View style={styles.overlayActions}>
                <Pressable
                  ...
                  disabled={Boolean(pendingDeleteId && globalLoading)}
                ...
                Alert.alert(
                  ...
                  [
                    { text: 'Batal', style: 'cancel' },
                    {
                      text: 'Ya, Ubah',
                      onPress: () =>
                        setPropertyStatus(resolvePropertyDocId(item), nextId),
                    },
                  ],
                );
              }}
          ...
          <View style={styles.heroAction}>
            ...
            >
              <MaterialCommunityIcons
                ...
                color={colors.PRIMARY}
              />
              <Text style={styles.dummyButtonText}>Dummy</Text>
            </Pressable>
          </View>
        </View>
        ...
        {pendingDeleteId && globalLoading && (
          <View style={styles.deleteLoadingOverlay}>
            <View style={styles.deleteLoadingCard}>
              ...
              <Text style={styles.deleteLoadingTitle}>Menghapus properti...</Text>
              <Text style={styles.deleteLoadingSubtitle}>
...
const createStyles = colors =>
  StyleSheet.create({
    ...
    deleteLoadingOverlay: {
      ...
      backgroundColor:
        colors.BACKGROUND === '#0D1B2D' ? 'rgba(8, 14, 26, 0.58)' : '#0B1E3A40',
      alignItems: 'center',
      justifyContent: 'center',
      zIndex: 20,
    },
    deleteLoadingCard: {
      minWidth: 220,
      backgroundColor:
        colors.BACKGROUND === '#0D1B2D' ? '#12213D' : colors.WHITE,
      borderRadius: 18,
      paddingVertical: 18,
    ...
    dummyButton: {
      ...
      paddingVertical: 8,
      backgroundColor:
        colors.BACKGROUND === '#0D1B2D' ? '#12213A' : colors.WHITE,
    },
    dummyButtonText: {
      color: colors.PRIMARY,
      fontFamily: Fonts.fontSemiBold,
