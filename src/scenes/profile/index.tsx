...
function ProfileScreen() {
  ...
  const stickyHeaderSurface = React.useMemo(
    () => ({
      backgroundColor: isDark ? 'rgba(13,27,45,0.98)' : '#F2F6FD',
      borderBottomColor: isDark ? 'rgba(255,255,255,0.08)' : '#D9E4F2',
    }),
    [isDark],
  );
  const profileHeaderCardStyle = React.useMemo(
    () => ({
      backgroundColor: isDark ? 'rgba(255,255,255,0.03)' : '#FFFFFFCC',
      borderColor: isDark ? 'rgba(255,255,255,0.08)' : '#DDE7F5',
    }),
    [isDark],
  );
  const profileTextStyle = React.useMemo(
    () => ({
      userNameColor: isDark ? Colors.WHITE : Colors.TEXT,
      userEmailColor: isDark ? Colors.WHITE_80 : Colors.GRAY_DARK,
      contactTextColor: isDark ? Colors.WHITE : Colors.TEXT,
      contactIconColor: isDark ? Colors.SUCCESS : Colors.PRIMARY,
      phoneGradient: isDark
        ? Colors.GRADIENT_EMERALD
        : ['#E8F7F0', '#D9F0E8'],
      waGradient: isDark
        ? Colors.GRADIENT_PURPLE_HAZE
        : ['#F0EBFF', '#E5DEFF'],
    }),
    [isDark],
  );
  const stickyBarStyle = React.useMemo(
    () => ({
      backgroundColor: isDark ? 'rgba(255,255,255,0.06)' : '#FFFFFFE8',
      borderColor: isDark ? 'rgba(255,255,255,0.10)' : '#D8E3F2',
      editBackgroundColor: isDark ? Colors.PRIMARY : '#2F7FEA',
      avatarFallbackColor: isDark ? Colors.PRIMARY_80 : '#4D8EEB',
    }),
    [isDark],
  );
  ...
  return (
    ...
    >
      ...
      >
        <View style={[styles.stickyHeaderContainer, stickyHeaderSurface]}>
          <View style={[styles.stickyBar, stickyBarStyle]}>
            <TouchableOpacity
              onPress={() => {
                if (user?.photoURL) {
              ...
              unflex
              style={styles.stickyAvatarTouch}
            >
              {user?.photoURL ? (
                <FastImage
                ...
                <View
                  style={[
                    styles.stickyAvatarFallback,
                    { backgroundColor: stickyBarStyle.avatarFallbackColor },
                  ]}
                >
                  <Text style={styles.stickyAvatarInitial}>{profileInitial}</Text>
                </View>
              )}
            </TouchableOpacity>
            <View style={styles.stickyIdentity}>
              <Text style={[styles.stickyName, { color: profileTextStyle.userNameColor }]}>
                {displayName}
              </Text>
              <Text
                style={[styles.stickyEmail, { color: profileTextStyle.userEmailColor }]}
                numberOfLines={1}
              >
                {emailOrContact}
              </Text>
            ...
            <TouchableOpacityDefault
              ...
              style={[
                styles.stickyEditButton,
                { backgroundColor: stickyBarStyle.editBackgroundColor },
              ]}
              activeOpacity={0.85}
            >
              <Ionicons
        ...
        <View style={[styles.profileHeader, profileHeaderCardStyle]}>
          ...
          <Text style={[styles.userName, { color: profileTextStyle.userNameColor }]}>
            {displayName}
          </Text>
          <Text
            style={[styles.userEmail, { color: profileTextStyle.userEmailColor }]}
          >
            {emailOrContact}
          </Text>
          <View style={styles.contactRow}>
            {phoneNumber ? (
              <LinearGradient
                colors={profileTextStyle.phoneGradient}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
              ...
              >
                <Ionicons
                  name="call"
                  size={FontSize.FONT_SIZE_18}
                  color={profileTextStyle.contactIconColor}
                  style={styles.contactIcon}
                />
                <Text
                  style={[styles.contactText, { color: profileTextStyle.contactTextColor }]}
                >
                  {phoneNumber}
                </Text>
              </LinearGradient>
            ...
            {whatsappNumber ? (
              <LinearGradient
                colors={profileTextStyle.waGradient}
                start={{ x: 0, y: 0 }}
                end={{ x: 1, y: 1 }}
                style={styles.contactChip}
              >
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    ...
    paddingBottom: Sizes.SIZE_50 + Sizes.SIZE_50,
    backgroundColor: Colors.WHITE,
  },
  stickyHeaderContainer: {
    marginHorizontal: -Sizes.SIZE_20,
    paddingHorizontal: Sizes.SIZE_20,
    paddingTop: Sizes.CUSTOM_SIZE(8),
    paddingBottom: Sizes.CUSTOM_SIZE(8),
    borderBottomWidth: 1,
    zIndex: 10,
    elevation: 8,
  },
  stickyBar: {
    minHeight: Sizes.CUSTOM_SIZE(66),
    borderRadius: Sizes.CUSTOM_SIZE(18),
    paddingHorizontal: Sizes.CUSTOM_SIZE(12),
    flexDirection: 'row',
    alignItems: 'center',
    borderWidth: 1,
  },
  stickyAvatarTouch: {
    width: Sizes.CUSTOM_SIZE(44),
    height: Sizes.CUSTOM_SIZE(44),
    borderRadius: Sizes.CUSTOM_SIZE(22),
    overflow: 'hidden',
  ...
  stickyEditButton: {
    width: Sizes.CUSTOM_SIZE(36),
    height: Sizes.CUSTOM_SIZE(36),
    borderRadius: Sizes.CUSTOM_SIZE(18),
    alignItems: 'center',
    justifyContent: 'center',
  },
